...
export default function PythonDeleteForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await fetch('http://localhost:8080/python_delete', {
                method: 'POST',
                headers: {
    ...
    return (
        <form className='max-w-2xl mx-auto bg-emerald-950  rounded-lg p-6 space-y-6 m-5 mr-4 ml-4 transition delay-50 duration-300 ease-in-out hover:-translate-y-1 hover:scale-105' onSubmit={handleSubmit}>
            <h2 className="text-2xl font-semibold text-yellow-500">Delete Python Project</h2>
            <div>
